...
class _AdminHomeScreenState extends State<AdminHomeScreen> {
  ...
  // [ADDED] Track errors per section for better debugging
  String? _statsError;
  ...
  Future<void> _fetchStats() async {
    ...
    try {
      ...
      // [DEBUG] Confirm token and user before making requests
      final userRaw = prefs.getString('user') ?? '{}';
      final user = jsonDecode(userRaw);
      print('[DASHBOARD] Token: $token');
      print('[DASHBOARD] municipalityId: ${user['municipalityId']}');

      if (token.isEmpty) {
        setState(() {
          _loadingStats = false;
          _statsError = 'Token manquant — reconnectez-vous';
        });
        return;
      }
      ...
      // [FIX] Run requests individually so one failure doesn't kill all stats
      // Previously: Future.wait — if one failed, all stats were lost silently
      http.Response? usersRes;
      http.Response? sigsRes;
      http.Response? agentsRes;
      http.Response? catsRes;

      try {
        usersRes = await http
            .get(Uri.parse(ApiConstants.getAllUsers), headers: headers)
            .timeout(const Duration(seconds: 10));
        print('[DASHBOARD] Users status: ${usersRes.statusCode}');
        print('[DASHBOARD] Users body: ${usersRes.body}');
      } catch (e) {
        print('[DASHBOARD] Users request failed: $e');
      }

      try {
        sigsRes = await http
            .get(Uri.parse(ApiConstants.getAllSignalements), headers: headers)
            .timeout(const Duration(seconds: 10));
        print('[DASHBOARD] Signalements status: ${sigsRes.statusCode}');
        print('[DASHBOARD] Signalements body: ${sigsRes.body}');
      } catch (e) {
        print('[DASHBOARD] Signalements request failed: $e');
      }

      try {
        agentsRes = await http
            .get(Uri.parse(ApiConstants.getAllAgents), headers: headers)
            .timeout(const Duration(seconds: 10));
        print('[DASHBOARD] Agents status: ${agentsRes.statusCode}');
        print('[DASHBOARD] Agents body: ${agentsRes.body}');
      } catch (e) {
        print('[DASHBOARD] Agents request failed: $e');
      }

      try {
        catsRes = await http
            .get(Uri.parse(ApiConstants.getAllCategories), headers: headers)
            .timeout(const Duration(seconds: 10));
        print('[DASHBOARD] Categories status: ${catsRes.statusCode}');
      } catch (e) {
        print('[DASHBOARD] Categories request failed: $e');
      }
      ...
      if (usersRes != null && usersRes.statusCode == 200) {
        ...
        print('[DASHBOARD] Total users in municipality: $_totalUsers');
      } else if (usersRes != null) {
        print(
          '[DASHBOARD] Users error: ${usersRes.statusCode} ${usersRes.body}',
        );
      }

      // ── Parse signalements ───────────────────────────────
      if (sigsRes != null && sigsRes.statusCode == 200) {
        final data = jsonDecode(sigsRes.body);
        final list = data['data'] as List;
        ...
        _catCounts = counts;
        print(
          '[DASHBOARD] Total signalements in municipality: $_totalSignalements',
        );
      } else if (sigsRes != null) {
        print(
          '[DASHBOARD] Signalements error: ${sigsRes.statusCode} ${sigsRes.body}',
        );
      }

      // ── Parse agents ─────────────────────────────────────
      if (agentsRes != null && agentsRes.statusCode == 200) {
        final data = jsonDecode(agentsRes.body);
        _totalAgents = (data['data'] as List).length;
        print('[DASHBOARD] Total agents in municipality: $_totalAgents');
      } else if (agentsRes != null) {
        print(
          '[DASHBOARD] Agents error: ${agentsRes.statusCode} ${agentsRes.body}',
        );
      }
      ...
      print('[DASHBOARD] Fatal error in _fetchStats: $e');
      setState(() {
        _loadingStats = false;
        _statsError = 'Erreur de chargement: $e';
      });
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // [ADDED] Error banner — shows if any request failed
              if (_statsError != null)
                Padding(
              ...
              if (_statsError != null) const SizedBox(height: 10),
